...
final readingTrackerRepositoryProvider = Provider<ReadingTrackerRepository>((ref) {
  return ReadingTrackerRepository(ref.watch(databaseProvider).value!);
});

class ReadingTrackerRepository {
  final Database _db;

  ReadingTrackerRepository(this._db);
  ...
  Future<ReadingSession?> getSession(int id) async {
  ...
  Future<List<ReadingSession>> getSessionsByBook(String bookTitle) async {
    final maps = await _db.query(
      'reading_sessions',
      where: 'book_title = ?',
      whereArgs: [bookTitle],
